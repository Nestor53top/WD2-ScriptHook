...
LUALIB_API int (luaL_checklstring)(lua_State *L, int numArg, size_t *len);
LUALIB_API const char *(luaL_optlstring)(lua_State *L, int numArg, const char *def, size_t *len);
...
typedef struct luaL_Buffer {
    ...
    int level;
    lua_State *L;
    char buffer[LUA_MAXCSTACK];
} luaL_Buffer;

#define luaL_putchar(B, c) ((void)((B)->p < ((B)->buffer+LUA_MAXCSTACK) ? (*(B)->p++ = (c)) : luaL_prepbuffer(B)))
